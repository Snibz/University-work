 ...
 *
 * Lisätty metodit set_value, get_value,
 * metodissa is_on_board SIZE muutettu side_of_boardiksi
 *
 */
...
{
    // returns tile's value to caller
    return value_;
}

// SIZE changed to board->sideofboard
bool NumberTile::is_on_board(Coords coords)
{
    return coords.first >= 0 and coords.first < board_->side_of_board_ and
           coords.second >= 0 and coords.second < board_->side_of_board_;
}
